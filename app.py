...
def sparkline_from_series(s: pd.Series, n=24) -> str:
    s = s.dropna().tail(n)
    if s.empty:
        return ""
    if s.nunique() == 1:
...
def style_df(df: pd.DataFrame):
    ...
    # Make sparkline stand out a bit
    if "Relative Strength 1M" in df.columns:
        sty = sty.set_properties(subset=["Relative Strength 1M"], **{"font-family": "monospace", "font-weight": "900"})

    return sty

# -----------------------------
# Manual inputs (formerly right panel)
# -----------------------------
DEFAULT_RIGHT = {
...
def style_grouped(df: pd.DataFrame):
    ...
    def _bold_headers(row):
        is_header = (row.get("Name", "") == "") and isinstance(row.get("Ticker", ""), str) and (row.get("Ticker") not in ALL_TICKERS_SET)
        return ["font-weight: 950;" if is_header else "" for _ in row.index]
...
all_sub_ticks = []
for v in list(SUBSECTOR_LEFT.values()) + list(SUBSECTOR_RIGHT.values()):
...
# ✅ CHANGE #1: combine into ONE long sub-sector block (right continues under left)
df_sub_all = pd.concat([df_left, df_right], ignore_index=True)
...
# ✅ KEEP indexes & sectors exactly the same, then show ONE big sub-sector table,
# ✅ then move Manual Inputs UNDER sub-sectors (not on the right)

st.markdown('<div class="section-title">Major U.S. Indexes</div>', unsafe_allow_html=True)
st.dataframe(style_df(df_major[show_cols]), use_container_width=True, height=330)

st.markdown('<div class="hr"></div>', unsafe_allow_html=True)

st.markdown('<div class="section-title">U.S. Sectors</div>', unsafe_allow_html=True)
st.dataframe(style_df(df_sectors[show_cols]), use_container_width=True, height=360)

st.markdown('<div class="hr"></div>', unsafe_allow_html=True)

st.markdown('<div class="section-title">U.S. Sub-Sectors / Industry Groups</div>', unsafe_allow_html=True)
st.dataframe(style_grouped(df_sub_all[show_cols]), use_container_width=True, height=1500)

st.markdown('<div class="hr"></div>', unsafe_allow_html=True)

# ✅ CHANGE #2: Manual inputs moved UNDER the sub-sector section
manual_inputs_ui()
